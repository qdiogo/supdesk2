...
");
	...
	if(isset($_POST['comentarios']) && $_POST['comentarios'] == 'comentarios'){
        $id_post =$_POST['id_post'];
        $curti = BD::conn()->prepare("INSERT INTO comentarios(id_usuario,id_post, mensagem) VALUES (?,?,?)");
		$curti->execute(array($_SESSION['id_user'],$_POST['id_post'],$_POST['mensagem']));
		header("Location: #$id_post");
...
");
      ...
      <div class="w3-container w3-card-2 w3-white w3-round w3-margin"><br>
        ...
        <form id="form_pesquisa" method="post" action="">
		 ...
		 <table width="100%">
		<?php
			$verifica = BD::conn()->prepare("SELECT *, (select nome from usuarios where id=id_usuario) as nomeusuario FROM `comentarios` WHERE `id_post` = ? and `id_usuario` = ?");
			$verifica->execute(array($id_post,$_SESSION['id_user']));
			
			while($row = $verifica->fetch()){
				echo "<tr><td>" . $row["mensagem"] . " - De:"  . $row["nomeusuario"] , "</td></tr>";
				
			}
		?>
		</table>
		<form id="form_pesquisa2" method="post" action="/sistema2/chat.php#24">
            <input type="hidden" name="id_post" value="<?php echo $id_post ?>" placeholder="">
            <table width="100%" >
				<td><input type="text" class="form-control" name="mensagem"  placeholder="Digitar um comentario"></td>
			
				<td><br><button type="submit" class="w3-btn w3-theme-d2 w3-margin-bottom" value='comentarios' VALUE="comentarios" name="comentarios" value="comentarios"><i class="fa fa-comment"></i> Comentar</button> </td>
			</table>
        </form>
