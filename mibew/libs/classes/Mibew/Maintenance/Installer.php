...
{
    /**
     * Minimal PHP version  Messenger works with.
     */
    ...
    /**
     * Checks if  Messenger is already installed or not.
     *
     * @return boolean
     */
    ...
    protected function prepopulateDatabase()
    {
        ...
        // Generate Unique ID for  Messenger Instance
        try {
            list($count) = $db->query(
                'SELECT COUNT(*) FROM {config} WHERE vckey = :key',
    ...
    protected function checkPhpVersion()
    {
        ...
        if ($current_version < self::MIN_PHP_VERSION) {
            $this->errors[] = getlocal(
                "PHP version is {0}, but  Messenger works with {1} and later versions.",
                array(
     ...
     *
     * If  Messenger is not installed yet boolean false will be returned.
     *
