    ...
    e.Views.ThreadsCollection = e.Views.CompositeBase.extend({
        ...
        updateStyles: function(e) {
            var t = this.collection,
                ...
                s = this;
            if (i.id) {
                var n = this.getQueueCode(i),
                    a = !1,
                    o = !1,
                    l = t.filter(function(e) {
                        return s.getQueueCode(e) == n
                    });
                if (l.length > 0 && (o = l[0].id == i.id, a = l[l.length - 1].id == i.id), e.lastStyles.length > 0) {
                    for (var r = 0, d = e.lastStyles.length; r < d; r++) e.$el.removeClass(e.lastStyles[r]);
                    e.lastStyles = []
                }
                var h = (n != this.QUEUE_BAN ? "in-" : "") + this.queueCodeToString(n);
                for (e.lastStyles.push(h), o && e.lastStyles.push(h + "-first"), a && e.lastStyles.push(h + "-last"), r = 0, d = e.lastStyles.length; r < d; r++) e.$el.addClass(e.lastStyles[r])
            }
        },
        ...
        threadAdded: function(t) {
            ...
            if (i === this.QUEUE_WAITING || i === this.QUEUE_PRIO) {
                var s = e.Objects.Models.page.get("mibewBasePath");
                "undefined" != typeof s && (s += "/sounds/new_user", e.Utils.playSound(s)), e.Objects.Models.page.get("showPopup") && this.once("render", function() {
