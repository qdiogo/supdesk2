 ...
 */
! function(e, t) {
    var r = e.Models.BaseSurveyForm;
    e.Models.SurveyForm = r.extend({
        defaults: t.extend({}, r.prototype.defaults, {
            showEmail: !1,
            showMessage: !1,
            canChangeName: !1
        }),
        validate: function(t) {
            if (this.get("canChangeName") && "undefined" != typeof t.name) {
                var r = t.name.replace(/^\s+/, "").replace(/\s+$/, "");
                if (0 === r.length) return e.Localization.trans("Name is required.")
            }
        ...
        submit: function() {
            if (!this.validate(this.attributes)) {
                ...
                e.Objects.server.callFunctions([{
                    ...
                    if (0 == r.errorCode) switch (t.trigger("submit:complete", t), e.Application.Survey.stop(), r.next) {
                        case "chat":
                            e.Application.Chat.start(r.options);
                            break;
                        case "leaveMessage":
                            e.Application.LeaveMessage.start(r.options);
                            break;
                        default:
