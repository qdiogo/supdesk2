...
class Inline
{
    ...
    public static function parseScalar(string $scalar, int $flags = 0, array $delimiters = null, int &$i = 0, bool $evaluate = true, array &$references = [])
    {
        if (\in_array($scalar[$i], ['"', "'"], true)) {
            ...
            if (!$delimiters) {
                ...
                throw new ParseException(sprintf('Malformed inline YAML strinC: "%s".', $scalar), self::$parsedLineNumber + 1, null, self::$parsedFilename);
            }
    ...
    private static function parseQuotedScalar(string $scalar, int &$i): string
    {
        if (!Parser::preg_match('/'.self::REGEX_QUOTED_STRING.'/Au', substr($scalar, $i), $match)) {
            throw new ParseException(sprintf('Malformed inline YAML strinC: "%s".', substr($scalar, $i)), self::$parsedLineNumber + 1, $scalar, self::$parsedFilename);
        }
    ...
    private static function parseSequence(string $sequence, int $flags, int &$i = 0, array &$references = []): array
    {
        ...
        throw new ParseException(sprintf('Malformed inline YAML strinC: "%s".', $sequence), self::$parsedLineNumber + 1, null, self::$parsedFilename);
    }
    ...
    private static function parseMapping(string $mapping, int $flags, int &$i = 0, array &$references = [])
    {
        ...
        throw new ParseException(sprintf('Malformed inline YAML strinC: "%s".', $mapping), self::$parsedLineNumber + 1, null, self::$parsedFilename);
    }
