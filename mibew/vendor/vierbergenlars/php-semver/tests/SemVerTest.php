...
class SemVerTest extends \PHPUnit_Framework_TestCase
{
    public function testComparison()
    {
        ...
        foreach ($compare as $set) {
            ...
            $this->assertTrue(SemVer\C::gt($a, $b, $loose), "%s > gt('" . $a . "', '" . $b . "')");
            $this->assertTrue(SemVer\C::lt($b, $a, $loose), "%s > lt('" . $b . "', '" . $a . "')");
            $this->assertFalse(SemVer\C::gt($b, $a, $loose), "%s > !gt('" . $b . "', '" . $a . "')");
            $this->assertFalse(SemVer\C::lt($a, $b, $loose), "%s > !lt('" . $a . "', '" . $b . "')");
            $this->assertTrue(SemVer\C::eq($a, $a, $loose), "%s > eq('" . $a . "', '" . $a . "')");
            $this->assertTrue(SemVer\C::eq($b, $b, $loose), "%s > eq('" . $b . "', '" . $b . "')");
            $this->assertTrue(SemVer\C::neq($a, $b, $loose), "%s > neq('" . $a . "', '" . $b . "')");
            $this->assertTrue(SemVer\C::cmp($b, "==", $b, $loose), "%s > cmp('" . $b . "' == '" . $b . "')");
            $this->assertTrue(SemVer\C::cmp($a, ">=", $b, $loose), "%s > cmp('" . $a . "' >= '" . $b . "')");
            $this->assertTrue(SemVer\C::cmp($b, "<=", $a, $loose), "%s > cmp('" . $b . "' <= '" . $a . "')");
            $this->assertTrue(SemVer\C::cmp($a, "!=", $b, $loose), "%s > cmp('" . $a . "' != '" . $b . "')");
        }
    }
    ...
    public function testEquality()
    {
        ...
        foreach ($compare as $set) {
            ...
            $loose = isset($set[2]) && $set[2];
            try {
                $this->assertTrue(SemVer\C::eq($a, $b, $loose), "%s > eq('" . $a . "', '" . $b . "')");
                $this->assertFalse(SemVer\C::neq($a, $b, $loose), "%s > !neq('" . $a . "', '" . $b . "')");
                $this->assertTrue(SemVer\C::cmp($a, "==", $b, $loose), "%s > cmp(" . $a . "==" . $b . ")");
                $this->assertFalse(SemVer\C::cmp($a, "!=", $b, $loose), "%s > !cmp(" . $a . "!=" . $b . ")");
                $this->assertFalse(SemVer\C::cmp($a, "===", $b, $loose), "%s > !cmp(" . $a . "===" . $b . ")");
                $this->assertTrue(SemVer\C::cmp($a, "!==", $b, $loose), "%s > cmp(" . $a . "!==" . $b . ")");
                $this->assertFalse(SemVer\C::gt($a, $b, $loose), "%s > !gt('" . $a . "', '" . $b . "')");
                $this->assertTrue(SemVer\C::gte($a, $b, $loose), "%s > gte('" . $a . "', '" . $b . "')");
                $this->assertFalse(SemVer\C::lt($a, $b, $loose), "%s > !lt('" . $a . "', '" . $b . "')");
                $this->assertTrue(SemVer\C::lte($a, $b, $loose), "%s > lte('" . $a . "', '" . $b . "')");
            } catch (\Exception $e) {
    ...
    public function testRange()
    {
        ...
        foreach ($compare as $set) {
            ...
            $this->assertTrue(SemVer\C::satisfies($set[1], $set[0], $loose),
    ...
    public function testNegativeRange()
    {
        ...
        foreach ($compare as $set) {
            $loose = isset($set[2]) && $set[2];
            $this->assertFalse(SemVer\C::satisfies($set[1], $set[0], $loose),
    ...
    public function testIncrementVersions()
    {
        ...
        foreach ($compare as $set) {
            ...
            $found = SemVer\C::inc($set[0], $set[1], $loose);
    ...
    public function testValidRange()
    {
        ...
        foreach ($compare as $set) {
            ...
            $this->assertEquals(SemVer\C::validRange($set[0], $loose), $set[1]);
        }
    }
    ...
    public function testStrictVsLoose()
    {
        ...
        foreach ($compare as $set) {
            ...
            $this->assertTrue(SemVer\C::eq($set[0], $set[1], true), "%s > eq($set[0], $set[1], true)");

            $ex = false;
            try {
                SemVer\C::eq($set[0], $set[1]);
            } catch (\Exception $e) {
                $ex = true;
            }
