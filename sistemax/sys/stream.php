...
if(isset($_GET)){
	...
	$pegaOnline = BD::conn()->prepare("SELECT DISTINCT usuarios.id,foto,nome,horario,limite,id_amigo1,id_amigo2,blocks FROM `usuarios` left join amigos ON usuarios.id=amigos.id_amigo1 or usuarios.id=amigos.id_amigo2 WHERE (usuarios.id != $userOnline ) AND id_amigo1=$userOnline  OR id_amigo2=$userOnline  AND (usuarios.id != $userOnline ) ORDER BY `id` DESC");
	$pegaOnline->execute();
	while($onlines = $pegaOnline->fetch()){
		$foto = ($onlines['foto'] == '') ? '../img/default.jpg' : '../img/'.$onlines['foto'];
