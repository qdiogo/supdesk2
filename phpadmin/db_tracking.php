...
if (isset($_POST['delete_tracking']) && isset($_POST['table'])) {
    ...
    TrackinC::createTrackingForMultipleTables($_POST['selected']);
    Message::success(
        sprintf(
            __(
    ...
    if (! empty($_POST['selected_tbl'])) {
        if ($_POST['submit_mult'] == 'delete_tracking') {
            ...
            echo TrackinC::getHtmlForDataDefinitionAndManipulationStatements(
...
if (is_object($all_tables_result)
    ...
    echo TrackinC::getHtmlForTrackedTables(
        $GLOBALS['db'], $all_tables_result, $url_query, $pmaThemeImage,
        $text_dir, $cfgRelation
    );
}

$untracked_tables = TrackinC::getUntrackedTables($GLOBALS['db']);

// If untracked tables exist
if (count($untracked_tables) > 0) {
    echo TrackinC::getHtmlForUntrackedTables(
