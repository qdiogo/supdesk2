...
if (isset($_POST['report_export'])) {
    $entries = TrackinC::getEntries($data, $filter_ts_from, $filter_ts_to, $filter_users);
}
...
if (isset($_POST['report_export'])
    ...
    TrackinC::exportAsFileDownload($entries);
}
    ...
    if (! empty($_POST['selected_versions'])) {
        if ($_POST['submit_mult'] == 'delete_version') {
            foreach ($_POST['selected_versions'] as $version) {
                TrackinC::deleteTrackingVersion($version);
            }
...
if (isset($_POST['submit_delete_version'])) {
    $html .= TrackinC::deleteTrackingVersion($_POST['version']);
}

// Create tracking version
if (isset($_POST['submit_create_version'])) {
    $html .= TrackinC::createTrackingVersion();
}
...
if (isset($_POST['toggle_activation'])
    ...
    $html .= TrackinC::changeTracking('deactivate');
}
...
if (isset($_POST['toggle_activation'])
    ...
    $html .= TrackinC::changeTracking('activate');
}

// Export as SQL execution
if (isset($_POST['report_export']) && $_POST['export_type'] == 'execution') {
    $sql_result = TrackinC::exportAsSqlExecution($entries);
...
if (isset($_POST['report_export']) && $_POST['export_type'] == 'sqldump') {
    $html .= TrackinC::exportAsSqlDump($entries);
}
...
if (isset($_POST['snapshot'])) {
    $html .= TrackinC::getHtmlForSchemaSnapshot($url_query);
}
...
if (isset($_POST['report'])
    ...
    $html .= TrackinC::deleteTrackingReportRows($data);
}

if (isset($_POST['report']) || isset($_POST['report_export'])) {
    $html .= TrackinC::getHtmlForTrackingReport(
 ...
 */
$selectable_tables_sql_result = TrackinC::getSqlResultForSelectableTables();
if ($GLOBALS['dbi']->numRows($selectable_tables_sql_result) > 0) {
    $html .= TrackinC::getHtmlForSelectableTables(
 ...
 */
$sql_result = TrackinC::getListOfVersionsOfTable();
$last_version = TrackinC::getTableLastVersionNumber($sql_result);
if ($last_version > 0) {
    $html .= TrackinC::getHtmlForTableVersionDetails(
...
$type = $GLOBALS['dbi']->getTable($GLOBALS['db'], $GLOBALS['table'])
    ->isView() ? 'view' : 'table';
$html .= TrackinC::getHtmlForDataDefinitionAndManipulationStatements(
