<?php 
if (empty($_GET["LOCAL"])){
	require "sessaotecnico87588834.php";
	$CNPJ="";
...
<?php 
	...
	$servidor = "192.168.15.2:F:\SGBD\SUPDESK\'$CNPJ\pessoal.fdb";

	if (!($conexao=ibase_connect(str_replace("'", "", $servidor), 'SYSDBA', 's@bia#:)ar@ra2021Ga','ISO8859_1', '9000', '1')))
	die('Erro ao conectar: ' .  ibase_errmsg());

	$servidor = "192.168.15.2:F:\SGBD\SUPDESK\CONTROLE.FDB";
...
function tirarAcentos($string) {
    // força UTF-8 (detecção automática do que veio)
...
function rtf2text($filename) {
	...
	function numerocelular($numero, $mensagem) {
		...
		// separa DDD e restante
		$ddd   = substr($numero, 0, 2);
		$resto = substr($numero, 2);

		// insere 9 se não tiver
		if (substr($resto, 0, 1) !== '9') {
			$resto = '9' . $resto;
		}
		
		$numerox = '55' . $ddd . $resto;

		// monta número final no formato do WhatsApp
